...
/** Manages which of a fixed catalog of page sections are shown, and in what order. */
export function SectionBlocksField({ catalog, value, onChange }: Props) {
  ...
  const available = catalog.filter(b => !value.includes(b.type))
  ...
  return (
    <div className="bg-white rounded-xl border border-stone/20 p-6 space-y-3">
      <div>
        ...
        <p className="text-xs text-stone mt-0.5">Add, remove or reorder the sections on this page.</p>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button type="button" onClick={() => move(type, -1)} disabled={i === 0}
                  className="p-1 rounded hover:bg-stone/20 disabled:opacity-30 transition-colors">
                  <ChevronUp className="h-3.5 w-3.5" />
                </button>
                <button type="button" onClick={() => move(type, 1)} disabled={i === value.length - 1}
                  className="p-1 rounded hover:bg-stone/20 disabled:opacity-30 transition-colors">
      ...
      <div className="relative">
        ...
        >
          <Plus className="h-4 w-4" /> {available.length ? 'Add section' : 'All sections added'}
        </button>
        ...
        <AnimatePresence>
          {showPicker && available.length > 0 && (
            <motion.div
              ...
              transition={{ duration: 0.15 }}
              className="absolute top-full mt-2 left-0 right-0 bg-white rounded-xl shadow-xl border border-stone/20 p-3 grid grid-cols-2 sm:grid-cols-3 gap-2 z-20"
            >
              {available.map(bt => (
                <button
                  key={bt.type}
